...
class ModelModuleComboProducts extends Model {
	...
	public function updateStatus($combo_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "combo_setting WHERE combo_id = '". $combo_id . "'");
		$result = $query->row;
		if ($result['status'] == 1) $this->db->query("UPDATE ". DB_PREFIX ."combo_setting SET status = 0 WHERE combo_id = '". (int)$combo_id ."'");
		else $this->db->query("UPDATE ". DB_PREFIX ."combo_setting SET status = 1 WHERE combo_id = '". (int)$combo_id ."'");
		$this->cache->delete('combo');
	}	
}
?>