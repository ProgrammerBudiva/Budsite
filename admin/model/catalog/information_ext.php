<?php
class ModelCatalogInformationExt extends Model {
	protected $count = 0;
	...
	public function quickEditInformation($information_id, $column, $value, $lang_id=null, $data=null) {
		...
		if (in_array($column, $editable)) {
			...
			foreach ((array)$data['description'] as $language_id => $value) {
				$this->db->query("UPDATE " . DB_PREFIX . "information_description SET description = '" . $this->db->escape($value['description']) . "' WHERE information_id = '" . (int)$information_id . "' AND language_id = '" . (int)$language_id . "'");
			}
			$result = 1;
		}
