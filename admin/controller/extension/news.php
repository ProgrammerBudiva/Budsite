...
class ControllerExtensionNews extends Controller {
  ...
  public function index() {
    ...
    $data['results'] = sprintf(
      ...
      ((($page - 1) * $this->config->get('config_limit_admin')) > ($total - $this->config->get('config_limit_admin'))) ? $total :
    ...
    foreach ($all_news as $news) {
      $data['all_news'][] = array(
        ...
        'edit'              => $this->url->link(
          'extension/news/edit',
          'news_id=' . $news['news_id'] . '&token=' . $this->session->data['token'] . $url,
          'SSL'
        ),
      );
    }
  ...
  protected function form() {
    ...
    if (isset($this->request->get['news_id'])) {
      $data['action'] =
        $this->url->link('extension/news/edit', '&news_id=' . $this->request->get['news_id'] . '&token=' . $this->session->data['token'], 'SSL');
    }
    else {
