<?php
class ControllerMarketingCouponExt extends Controller {
	...
	public function __construct($registry) {
		...
		$this->load->model('marketing/coupon_ext');

		$this->load->language('marketing/coupon');
		$this->load->language('marketing/qe_general');
		$this->load->language('marketing/coupon_ext');
	...
	protected function getList() {
		...
		$results = $this->model_marketing_coupon_ext->getCoupons($filter_data);

		$coupon_total = $this->model_marketing_coupon_ext->getTotalCoupons();
				...
				switch ($action) {
					case 'edit':
						$_buttons[] = array(
							...
							'url'   => html_entity_decode($this->url->link('marketing/coupon/edit', '&coupon_id=' . $result['coupon_id'] . '&token=' . $this->session->data['token'] . $url, 'SSL'), ENT_QUOTES, 'UTF-8'),
							'icon'  => $attr['icon'],
							'name'  => null,
			...
			if (!is_array($columns)) {
				...
				foreach ($columns as $column => $attr) {
					if (in_array($column, array('logged', 'shipping'))) {
						...
						$products = $this->model_marketing_coupon_ext->getCouponProducts($result['coupon_id']);
		...
		$data['update_url'] = html_entity_decode($this->url->link('marketing/coupon/quick_update', 'token=' . $this->session->data['token'], 'SSL'));
		$data['load_popup_url'] = html_entity_decode($this->url->link('marketing/coupon/load_popup', 'token=' . $this->session->data['token'], 'SSL'));
		$data['refresh_url'] = html_entity_decode($this->url->link('marketing/coupon/refresh_data', 'token=' . $this->session->data['token'], 'SSL'));
		...
		$data['sorts'] = array();
		foreach ($this->config->get('aqe_marketing_coupons') as $column => $attr) {
			$data['sorts'][$column] = $this->url->link('marketing/coupon', 'token=' . $this->session->data['token'] . '&sort=' . $attr['sort'] . $url, 'SSL');
		}
		...
		$this->response->setOutput($this->load->view('marketing/aqe/coupon_ext_list.tpl', $data));
	}
	...
	public function load_popup() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->validateLoadPopup($this->request->post)) {
			...
			switch ($data['parameter']) {
				...
				case 'products':
					$data['text_autocomplete'] = $this->language->get('text_autocomplete');
					$data['autocomplete'] = html_entity_decode($this->url->link('catalog/product/autocomplete', 'token=' . $this->session->data['token'] . '&filter_name=', 'SSL'));
					$data['item_label'] = 'name';
					$data['item_value'] = 'product_id';
					$items = array();
					$products = $this->model_marketing_coupon_ext->getCouponProducts($data['i_id']);
				...
				case 'categories':
					...
					$data['text_autocomplete'] = $this->language->get('text_autocomplete');
					$data['autocomplete'] = html_entity_decode($this->url->link('catalog/category/autocomplete', 'token=' . $this->session->data['token'] . '&filter_name=', 'SSL'));
					$data['item_label'] = 'name';
		...
		$response['popup'] = $this->load->view('marketing/aqe/quick_edit_form2.tpl', $data);
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->validateRefreshData($this->request->post)) {
			...
			foreach ($this->request->post['data'] as $column => $coupons) {
				foreach ($coupons as $id) {
					switch ($column) {
						case 'products':
							$products = $this->model_marketing_coupon_ext->getCouponProducts($id);
	...
	public function quick_update() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->validateUpdateData($this->request->post)) {
			...
			foreach ((array)$id as $_id) {
				if ($this->model_marketing_coupon_ext->quickEditCoupon($_id, $column, $value, $lang_id, $this->request->post)) {
					$results['done'][] = $_id;
				} else {
					$results['failed'][] = $_id;
				}
			...
			if ($results['done']) {
				...
				if (in_array($column, array('product', 'category'))) {
					...
					$products = $this->model_marketing_coupon_ext->getCouponProducts($id[0]);
					if ($products)
