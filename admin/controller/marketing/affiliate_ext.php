<?php
class ControllerMarketingAffiliateExt extends Controller {
	...
	public function __construct($registry) {
		...
		$this->load->model('marketing/affiliate_ext');

		$this->load->language('marketing/affiliate');
		$this->load->language('marketing/qe_general');
		$this->load->language('marketing/affiliate_ext');

		if (!$this->config->get('aqe_installed') || !$this->config->get('aqe_status')) {
			$this->response->redirect($this->url->link('marketing/affiliate', 'token=' . $this->session->data['token'], 'SSL'));
		}
	}
	...
	protected function getList() {
		...
		$results = $this->model_marketing_affiliate_ext->getAffiliates($filter_data);

		$affiliate_total = $this->model_marketing_affiliate_ext->getTotalAffiliates();
				...
				switch ($action) {
					case 'edit':
						$_buttons[] = array(
							...
							'url'   => html_entity_decode($this->url->link('marketing/affiliate/edit', '&affiliate_id=' . $result['affiliate_id'] . '&token=' . $this->session->data['token'] . $url, 'SSL'), ENT_QUOTES, 'UTF-8'),
							'icon'  => $attr['icon'],
							'name'  => null,
					...
					case 'approve':
						$_buttons[] = array(
							...
							'action'=> $action,
							'title' => $this->language->get('action_' . $action),
							'url'   => !$result['approved'] ? html_entity_decode($this->url->link('marketing/affiliate/approve', '&affiliate_id=' . $result['affiliate_id'] . '&token=' . $this->session->data['token'] . $url, 'SSL'), ENT_QUOTES, 'UTF-8') : '',
							'icon'  => $attr['icon'],
							'name'  => null,
							'rel'   => json_encode($attr['rel']),
							'class' => $attr['class'],
					...
					case 'unlock':
						...
						$_buttons[] = array(
							...
							'action'=> $action,
							'title' => $this->language->get('action_' . $action),
							'url'   => ($login_info && $login_info['total'] >= $this->config->get('config_login_attempts')) ? html_entity_decode($this->url->link('marketing/affiliate/unlock', '&email=' . $result['email'] . '&token=' . $this->session->data['token'] . $url, 'SSL'), ENT_QUOTES, 'UTF-8') : '',
							'icon'  => $attr['icon'],
							'name'  => null,
							'rel'   => json_encode($attr['rel']),
							'class' => $attr['class'],
		...
		$data['update_url'] = html_entity_decode($this->url->link('marketing/affiliate/quick_update', 'token=' . $this->session->data['token'], 'SSL'));
		$data['load_popup_url'] = html_entity_decode($this->url->link('marketing/affiliate/load_popup', 'token=' . $this->session->data['token'], 'SSL'));
		$data['load_zone_url'] = html_entity_decode($this->url->link('marketing/affiliate/load_zone', 'token=' . $this->session->data['token'], 'SSL'));
		...
		$data['sorts'] = array();
		foreach ($this->config->get('aqe_marketing_affiliates') as $column => $attr) {
			$data['sorts'][$column] = $this->url->link('marketing/affiliate', 'token=' . $this->session->data['token'] . '&sort=' . $attr['sort'] . $url, 'SSL');
		}
		...
		$this->response->setOutput($this->load->view('marketing/aqe/affiliate_ext_list.tpl', $data));
	}
	...
	public function quick_update() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->validateUpdateData($this->request->post)) {
			...
			if ($column == 'approve') {
				...
				foreach ((array)$id as $_id) {
					if ($this->model_marketing_affiliate_ext->quickEditAffiliate($_id, $column, $value, $lang_id, $this->request->post)) {
						$results['done'][] = $_id;
					} else {
						$results['failed'][] = $_id;
					}
