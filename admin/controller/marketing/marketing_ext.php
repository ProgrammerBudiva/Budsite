<?php
class ControllerMarketingMarketingExt extends Controller {
	...
	public function __construct($registry) {
		...
		$this->load->model('marketing/marketing_ext');

		$this->load->language('marketing/marketing');
		$this->load->language('marketing/qe_general');
		$this->load->language('marketing/marketing_ext');

		if (!$this->config->get('aqe_installed') || !$this->config->get('aqe_status')) {
			$this->response->redirect($this->url->link('marketing/marketing', 'token=' . $this->session->data['token'], 'SSL'));
		}
	}
	...
	protected function getList() {
		...
		$results = $this->model_marketing_marketing_ext->getMarketings($filter_data);

		$marketing_total = $this->model_marketing_marketing_ext->getTotalMarketings();
				...
				switch ($action) {
					case 'edit':
						$_buttons[] = array(
							...
							'url'   => html_entity_decode($this->url->link('marketing/marketing/edit', '&marketing_id=' . $result['marketing_id'] . '&token=' . $this->session->data['token'] . $url, 'SSL'), ENT_QUOTES, 'UTF-8'),
							'icon'  => $attr['icon'],
							'name'  => null,
		...
		$data['update_url'] = html_entity_decode($this->url->link('marketing/marketing/quick_update', 'token=' . $this->session->data['token'], 'SSL'));
		...
		$data['sorts'] = array();
		foreach ($this->config->get('aqe_marketing_campaigns') as $column => $attr) {
			$data['sorts'][$column] = $this->url->link('marketing/marketing', 'token=' . $this->session->data['token'] . '&sort=' . $attr['sort'] . $url, 'SSL');
		}
		...
		$this->response->setOutput($this->load->view('marketing/aqe/marketing_ext_list.tpl', $data));
	}
	...
	public function quick_update() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->validateUpdateData($this->request->post)) {
			...
			foreach ((array)$id as $_id) {
				if ($this->model_marketing_marketing_ext->quickEditMarketing($_id, $column, $value, $lang_id, $this->request->post)) {
					$results['done'][] = $_id;
				} else {
					$results['failed'][] = $_id;
				}
