 ...
 */
 ...
 class ControllerModuleComboProducts extends Controller {
    ...
    public function install() {
        ...
        $this->db->query("ALTER TABLE `" . DB_PREFIX . "combo_products` CONVERT TO CHARACTER SET utf8 COLLATE utf8_unicode_ci");
        $this->db->query("ALTER TABLE `" . DB_PREFIX . "combo_setting` CONVERT TO CHARACTER SET utf8 COLLATE utf8_unicode_ci");
        $this->db->query("ALTER TABLE `" . DB_PREFIX . "combo_category` CONVERT TO CHARACTER SET utf8 COLLATE utf8_unicode_ci");
		$this->db->query("CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "combo_category` (
          `combo_id` int(11) NOT NULL,
          `category_id` int(11) NOT NULL
        )");
		if (version_compare(VERSION, '2.0.0.0', '=')) {$code = 'group';} else {$code = 'code';}
	...
	protected function getList (){
		...
		foreach ($results as $result) {
			...
			if ($result['status'] == 1) {$status = 'ban'; $title = 'Enabled';}
			else {$status = 'check-circle';	$title = 'Disabled';}		
	...
	protected function getForm () {
		...
		if (isset($get_combo)) {
			...
			foreach ($get_products as $product_id) {
				...
				if ($product_info2['image']) $product_image = $this->model_tool_image->resize($product_info2['image'], 40, 40);
				else $product_image = $this->model_tool_image->resize('no_image.png', 40, 40);
				$combo_products[] = array (
					'product_name'	=> $product_info[$this->config->get('config_language_id')]['name'],
