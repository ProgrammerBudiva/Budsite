...
class ControllerModuleNews extends Controller {
	...
	public function index() {
		...
		if (isset($this->request->post['news_image_width'])) {
			$data['news_image_width'] = $this->request->post['news_image_width'];
		} else {
			$data['news_image_width'] = $this->config->get('news_image_width');
		}

		if (isset($this->request->post['news_image_height'])) {
			$data['news_image_height'] = $this->request->post['news_image_height'];
		} else {
			$data['news_image_height'] = $this->config->get('news_image_height');
		}
	...
	protected function validate() {
		...
		if (!$this->request->post['news_image_width'] || !$this->request->post['news_image_width'] || (int)$this->request->post['news_image_width'] <= 0 || (int)$this->request->post['news_image_height'] <= 0) {
			$this->error['error_dimension'] = $this->language->get('error_main_image_dimension');
		}

		if (!$this->request->post['news_image_height'] || !$this->request->post['news_image_height'] || (int)$this->request->post['news_image_height'] <= 0 || (int)$this->request->post['news_image_height'] <= 0) {
