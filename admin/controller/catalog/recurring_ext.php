<?php
class ControllerCatalogRecurringExt extends Controller {
	...
	public function __construct($registry) {
		...
		$this->load->model('catalog/recurring_ext');

		$this->load->language('catalog/recurring');
		$this->load->language('catalog/qe_general');
		$this->load->language('catalog/recurring_ext');

		if (!$this->config->get('aqe_installed') || !$this->config->get('aqe_status')) {
			$this->response->redirect($this->url->link('catalog/recurring', 'token=' . $this->session->data['token'], 'SSL'));
		}
	}
	...
	protected function getList() {
		...
		$data['column_frequency'] = $this->language->get('column_frequency');
		...
		$data['column_trial_price'] = $this->language->get('column_trial_price');
		$data['column_trial_frequency'] = $this->language->get('column_trial_frequency');
		$data['column_trial_duration'] = $this->language->get('column_trial_duration');
		$data['column_trial_cycle'] = $this->language->get('column_trial_cycle');
		...
		$columns = $this->config->get('aqe_catalog_recurrings');
		...
		$results = $this->model_catalog_recurring_ext->getRecurrings($filter_data);

		$recurring_total = $this->model_catalog_recurring_ext->getTotalRecurrings();
				...
				switch ($action) {
					case 'edit':
						$_buttons[] = array(
							...
							'url'   => html_entity_decode($this->url->link('catalog/recurring/edit', '&recurring_id=' . $result['recurring_id'] . '&token=' . $this->session->data['token'] . $url, 'SSL'), ENT_QUOTES, 'UTF-8'),
							'icon'  => 'pencil',
							'name'  => null,
			...
			if (!is_array($columns)) {
				...
				foreach ($columns as $column => $attr) {
					if (in_array($column, array('status', 'trial_status'))) {
						if ((int)$result[$column] || !$this->config->get('aqe_highlight_status')) {
		...
		$data['update_url'] = html_entity_decode($this->url->link('catalog/recurring/quick_update', 'token=' . $this->session->data['token'], 'SSL'));
		...
		$data['load_popup_url'] = html_entity_decode($this->url->link('catalog/recurring/load_popup', 'token=' . $this->session->data['token'], 'SSL'));
		...
		$data['sorts'] = array();
		foreach ($this->config->get('aqe_catalog_recurrings') as $column => $attr) {
			$data['sorts'][$column] = $this->url->link('catalog/recurring', 'token=' . $this->session->data['token'] . '&sort=' . $attr['sort'] . $url, 'SSL');
		}
		...
		$this->response->setOutput($this->load->view('catalog/aqe/recurring_ext_list.tpl', $data));
	}
	...
	public function quick_update() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->validateUpdateData($this->request->post)) {
			...
			foreach ((array)$id as $_id) {
				if ($this->model_catalog_recurring_ext->quickEditRecurring($_id, $column, $value, $lang_id, $this->request->post)) {
					$results['done'][] = $_id;
				} else {
					$results['failed'][] = $_id;
				}
