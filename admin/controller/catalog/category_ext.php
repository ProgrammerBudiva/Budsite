<?php
class ControllerCatalogCategoryExt extends Controller {
	...
	public function __construct($registry) {
		...
		$this->load->model('catalog/category_ext');

		$this->load->language('catalog/category');
		$this->load->language('catalog/qe_general');
		$this->load->language('catalog/category_ext');
	...
	protected function getList() {
		...
		$data['copy'] = $this->url->link('catalog/category/copy', 'token=' . $this->session->data['token'] . $url, 'SSL');
		$data['delete'] = $this->url->link('catalog/category/delete', 'token=' . $this->session->data['token'] . $url, 'SSL');
		$data['rebuild'] = $this->url->link('catalog/category/repair', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		if ($this->config->get('config_seo_url')) {
			$_url->addRewrite(new ControllerCommonSeoUrl($this->registry));
		}
		...
		foreach ($stores as $store) {
			...
			$data['stores'][$store['store_id']] = array(
				...
				'url'   => $store['url'],
				'ssl'   => $store['ssl'],
				'make'  => new Url($store['url'], $secure ? $store['url'] : $store['ssl'])
			);

			if ($this->config->get('config_seo_url')) {
				$data['stores'][$store['store_id']]['make']->addRewrite(new ControllerCommonSeoUrl($this->registry));
			}
		}
		...
		$results = $this->model_catalog_category_ext->getCategories($filter_data);

		$category_total = $this->model_catalog_category_ext->getTotalCategories();
				...
				switch ($action) {
					case 'edit':
						$_buttons[] = array(
							...
							'url'   => html_entity_decode($this->url->link('catalog/category/edit', '&category_id=' . $result['category_id'] . '&token=' . $this->session->data['token'] . $url, 'SSL'), ENT_QUOTES, 'UTF-8'),
							'icon'  => 'pencil',
							'name'  => null,
		...
		$data['update_url'] = html_entity_decode($this->url->link('catalog/category/quick_update', 'token=' . $this->session->data['token'], 'SSL'));
		$data['refresh_url'] = html_entity_decode($this->url->link('catalog/category/refresh_data', 'token=' . $this->session->data['token'], 'SSL'));
		...
		$data['load_popup_url'] = html_entity_decode($this->url->link('catalog/category/load_popup', 'token=' . $this->session->data['token'], 'SSL'));
		...
		if (in_array("parent", $displayed_columns)) {
			$data['_categories'] = $this->model_catalog_category_ext->getCategories(array('sort' => 'name'));
		...
		if (in_array("filter", $displayed_columns)) {
			$this->load->model('catalog/filter_ext');
			$data['_filters'] = $this->model_catalog_filter_ext->getFiltersByGroup();
		}
		...
		$data['sorts'] = array();
		foreach ($this->config->get('aqe_catalog_categories') as $column => $attr) {
			$data['sorts'][$column] = $this->url->link('catalog/category', 'token=' . $this->session->data['token'] . '&sort=' . $attr['sort'] . $url, 'SSL');
		}
		...
		$this->response->setOutput($this->load->view('catalog/aqe/category_ext_list.tpl', $data));
	}
	...
	public function filter() {
		...
		$results = $this->model_catalog_filter_ext->getFiltersByFilterGroupId($filter_group_id);
	...
	public function load_popup() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->validateLoadPopup($this->request->post)) {
			...
			switch ($data['parameter']) {
				...
				case "seo":
					...
					$data['languages'] = $this->model_localisation_language->getLanguages();
					$data['value'] = array();
					$keywords = $this->model_catalog_category_ext->getCategorySeoKeywords($data['i_id']);
				...
				case "filter":
					...
					foreach ($filter_groups as $filter_group) {
						$data['filters'] = array_merge($data['filters'], $this->model_catalog_filter_ext->getFiltersByFilterGroupId($filter_group['filter_group_id']));
					}
		...
		$response['popup'] = $this->load->view('catalog/aqe/quick_edit_form2.tpl', $data);
	...
	public function quick_update() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->validateUpdateData($this->request->post)) {
			...
			foreach ((array)$id as $_id) {
				if ($this->model_catalog_category_ext->quickEditCategory($_id, $column, $value, $lang_id, $this->request->post)) {
					$results['done'][] = $_id;
				} else {
					$results['failed'][] = $_id;
				}
			...
			if (isset($data['ids']) && count((array)$data['ids']) > 1) {
				...
				if (isset($data['value']) && $multilingual_seo) {
					foreach ((array)$data['value'] as $language_id => $value) {
						...
						if ($this->model_catalog_category_ext->urlAliasExists($id, $keyword, $language_id)) {
							$errors = true;
					...
					if ($this->model_catalog_category_ext->urlAliasExists($id, $keyword)) {
						$errors = true;
