<?php
class ControllerCatalogInformationExt extends Controller {
	...
	public function __construct($registry) {
		...
		$this->load->model('catalog/information_ext');

		$this->load->language('catalog/information');
		$this->load->language('catalog/qe_general');
		$this->load->language('catalog/information_ext');

		if (!$this->config->get('aqe_installed') || !$this->config->get('aqe_status')) {
			$this->response->redirect($this->url->link('catalog/information', 'token=' . $this->session->data['token'], 'SSL'));
		}
	}
	...
	protected function getList() {
		...
		$results = $this->model_catalog_information_ext->getInformations($filter_data);

		$information_total = $this->model_catalog_information_ext->getTotalInformations();
				...
				switch ($action) {
					case 'edit':
						$_buttons[] = array(
							...
							'url'   => html_entity_decode($this->url->link('catalog/information/edit', '&information_id=' . $result['information_id'] . '&token=' . $this->session->data['token'] . $url, 'SSL'), ENT_QUOTES, 'UTF-8'),
							'icon'  => 'pencil',
							'name'  => null,
		...
		$data['update_url'] = html_entity_decode($this->url->link('catalog/information/quick_update', 'token=' . $this->session->data['token'], 'SSL'));
		$data['refresh_url'] = html_entity_decode($this->url->link('catalog/information/refresh_data', 'token=' . $this->session->data['token'], 'SSL'));
		$data['load_popup_url'] = html_entity_decode($this->url->link('catalog/information/load_popup', 'token=' . $this->session->data['token'], 'SSL'));
		...
		$data['sorts'] = array();
		foreach ($this->config->get('aqe_catalog_information') as $column => $attr) {
			$data['sorts'][$column] = $this->url->link('catalog/information', 'token=' . $this->session->data['token'] . '&sort=' . $attr['sort'] . $url, 'SSL');
		}
		...
		$this->response->setOutput($this->load->view('catalog/aqe/information_ext_list.tpl', $data));
	}
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->validateLoadPopup($this->request->post)) {
			...
			switch ($data['parameter']) {
				...
				case "seo":
					...
					$data['languages'] = $this->model_localisation_language->getLanguages();
					$data['value'] = array();
					$keywords = $this->model_catalog_information_ext->getInformationSeoKeywords($data['i_id']);
	...
	public function quick_update() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->validateUpdateData($this->request->post)) {
			...
			foreach ((array)$id as $_id) {
				if ($this->model_catalog_information_ext->quickEditInformation($_id, $column, $value, $lang_id, $this->request->post)) {
					$results['done'][] = $_id;
				} else {
					$results['failed'][] = $_id;
				}
			...
			if (isset($data['ids']) && count((array)$data['ids']) > 1) {
				...
				if (isset($data['value']) && $multilingual_seo) {
					foreach ((array)$data['value'] as $language_id => $value) {
						...
						if ($this->model_catalog_information_ext->urlAliasExists($id, $keyword, $language_id)) {
							$errors = true;
					...
					if ($this->model_catalog_information_ext->urlAliasExists($id, $keyword)) {
						$errors = true;
