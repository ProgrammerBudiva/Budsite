<?php
class ControllerCatalogAttributeExt extends Controller {
	...
	public function __construct($registry) {
		...
		$this->load->model('catalog/attribute_ext');

		$this->load->language('catalog/attribute');
		$this->load->language('catalog/qe_general');
		$this->load->language('catalog/attribute_ext');

		if (!$this->config->get('aqe_installed') || !$this->config->get('aqe_status')) {
			$this->response->redirect($this->url->link('catalog/attribute', 'token=' . $this->session->data['token'], 'SSL'));
		}
	}
	...
	protected function getList() {
		...
		$columns = $this->config->get('aqe_catalog_attributes');
		...
		$results = $this->model_catalog_attribute_ext->getAttributes($filter_data);

		$attribute_total = $this->model_catalog_attribute_ext->getTotalAttributes();
				...
				switch ($action) {
					case 'edit':
						$_buttons[] = array(
							...
							'url'   => html_entity_decode($this->url->link('catalog/attribute/edit', '&attribute_id=' . $result['attribute_id'] . '&token=' . $this->session->data['token'] . $url, 'SSL'), ENT_QUOTES, 'UTF-8'),
							'icon'  => 'pencil',
							'name'  => null,
		...
		$data['update_url'] = html_entity_decode($this->url->link('catalog/attribute/quick_update', 'token=' . $this->session->data['token'], 'SSL'));
		$data['load_popup_url'] = html_entity_decode($this->url->link('catalog/attribute/load_popup', 'token=' . $this->session->data['token'], 'SSL'));
		...
		$data['sorts'] = array();
		foreach ($this->config->get('aqe_catalog_attributes') as $column => $attr) {
			$data['sorts'][$column] = $this->url->link('catalog/attribute', 'token=' . $this->session->data['token'] . '&sort=' . $attr['sort'] . $url, 'SSL');
		}
		...
		$this->response->setOutput($this->load->view('catalog/aqe/attribute_ext_list.tpl', $data));
	}
	...
	public function quick_update() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->validateUpdateData($this->request->post)) {
			...
			foreach ((array)$id as $_id) {
				if ($this->model_catalog_attribute_ext->quickEditAttribute($_id, $column, $value, $lang_id, $this->request->post)) {
					$results['done'][] = $_id;
				} else {
					$results['failed'][] = $_id;
				}
