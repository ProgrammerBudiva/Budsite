<?php
class ControllerCatalogFilterExt extends Controller {
	...
	public function __construct($registry) {
		...
		$this->load->language('catalog/filter_ext');

		if (!$this->config->get('aqe_installed') || !$this->config->get('aqe_status')) {
			$this->response->redirect($this->url->link('catalog/filter', 'token=' . $this->session->data['token'], 'SSL'));
		}
	}
	...
	protected function getList() {
		...
		$data['copy'] = $this->url->link('catalog/filter/copy', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		$results = $this->model_catalog_filter_ext->getFilterGroups($filter_data);

		$filter_total = $this->model_catalog_filter_ext->getTotalFilterGroups();
		...
		foreach ($results as $result) {
				...
				switch ($action) {
					case 'edit':
						$_buttons[] = array(
							...
							'url'   => html_entity_decode($this->url->link('catalog/filter/edit', '&filter_group_id=' . $result['filter_group_id'] . '&token=' . $this->session->data['token'] . $url, 'SSL'), ENT_QUOTES, 'UTF-8'),
							'icon'  => 'pencil',
							'name'  => null,
			...
			if (!is_array($columns)) {
				...
				foreach ($columns as $column => $attr) {
					// if ($column == 'filter') {
					// 	$fs = $this->model_catalog_filter_ext->getFiltersByFilterGroupId($result['filter_group_id']);
					// 	$filters = array();
		...
		$data['update_url'] = html_entity_decode($this->url->link('catalog/filter/quick_update', 'token=' . $this->session->data['token'], 'SSL'));
		$data['refresh_url'] = html_entity_decode($this->url->link('catalog/filter/refresh_data', 'token=' . $this->session->data['token'], 'SSL'));

		$data['load_popup_url'] = html_entity_decode($this->url->link('catalog/filter/load_popup', 'token=' . $this->session->data['token'], 'SSL'));
		...
		$data['sorts'] = array();
		foreach ($this->config->get('aqe_catalog_filters') as $column => $attr) {
			$data['sorts'][$column] = $this->url->link('catalog/filter', 'token=' . $this->session->data['token'] . '&sort=' . $attr['sort'] . $url, 'SSL');
		}
		...
		$this->response->setOutput($this->load->view('catalog/aqe/filter_ext_list.tpl', $data));
	}
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->validateRefreshData($this->request->post)) {
			...
			foreach ($this->request->post['data'] as $column => $filters) {
				foreach ($filters as $id) {
					switch ($column) {
						case 'filter':
							$fs = $this->model_catalog_filter_ext->getFiltersByFilterGroupId($id);
							$_filters = array();
							foreach($fs as $filter) {
	...
	public function quick_update() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->validateUpdateData($this->request->post)) {
			...
			foreach ((array)$id as $_id) {
				if ($this->model_catalog_filter_ext->quickEditFilter($_id, $column, $value, $lang_id, $this->request->post)) {
					$results['done'][] = $_id;
				} else {
					$results['failed'][] = $_id;
				}
