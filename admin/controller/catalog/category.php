...
class ControllerCatalogCategory extends Controller {
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
            ...
            $this->cache->delete('ocfilter.option.' . $this->request->get['category_id'] . '.' . $this->config->get('config_language_id'));
    ...
    $filter_data = array(
      ...
      'limit'       => $this->config->get('config_limit_admin'),
		...
		foreach ($results as $result) {
            ...
            if($result['type']){
                $type = $this->language->get('entry_lookbook');
            } else {
                $type = $this->language->get('entry_simple');
            }
	...
	protected function getForm() {
		...
		$data['entry_lower_price'] = $this->language->get('entry_lower_price');
		...
		$data['entry_simple'] = $this->language->get('entry_simple');
		$data['entry_lookbook'] = $this->language->get('entry_lookbook');
		$data['entry_layout'] = $this->language->get('entry_layout');

		$data['help_filter'] = $this->language->get('help_filter');
		$data['help_keyword'] = $this->language->get('help_keyword');
		$data['help_lower_price'] = $this->language->get('help_lower_price');
		...
		if (isset($this->error['lower_price'])) {
			$data['error_lower_price'] = $this->error['lower_price'];
		} else {
			$data['error_lower_price'] = '';
		}
    ...
    // сопутствующие товары для товаров этой категории
    $data['product_relateds'] = [];
    // категории, из которых берутся сопутствующие товары
    $data['category_relateds'] = [];
    // сопутствующие теговые категории
    $data['tagged_related_categories'] = [];

	  if (isset($this->request->get['category_id'])) {
			$relatedData = $this->model_catalog_category->getRelated($this->request->get['category_id']);
      $this->load->model('catalog/product');

      /* @var array $ids Array of product id
       * @return array
       */
      $prepareRelatedProducts = function($ids){
        $result = [];
      ...
      if (!empty($category_info) && !empty($category_info['tagged_related'])) {
        $tagged_related_categories = explode(',', $category_info['tagged_related']);
      }

      $data['product_relateds'] = $prepareRelatedProducts((array)$relatedData['products']);
      $data['category_relateds'] = $prepareCategoriesList((array)$relatedData['categories']);
      $data['limit_related'] = $relatedData['limit'] ? $relatedData['limit'] : 10;
      $data['tagged_related_categories'] = $prepareCategoriesList($tagged_related_categories);
		}
    ...
    if (isset($this->request->post['category_description'])) {
		...
		if (isset($this->request->post['lower_price'])) {
			$data['lower_price'] = $this->request->post['lower_price'];
		} elseif (!empty($category_info)) {
			$data['lower_price'] = $category_info['lower_price'];
		} else {
			$data['lower_price'] = '';
		}
