<?php
class ControllerCatalogReviewExt extends Controller {
	...
	public function __construct($registry) {
		...
		$this->load->model('catalog/review_ext');

		$this->load->language('catalog/review');
		$this->load->language('catalog/qe_general');
		$this->load->language('catalog/review_ext');

		if (!$this->config->get('aqe_installed') || !$this->config->get('aqe_status')) {
			$this->response->redirect($this->url->link('catalog/review', 'token=' . $this->session->data['token'], 'SSL'));
		}
	}
	...
	protected function getList() {
		...
		$columns = $this->config->get('aqe_catalog_reviews');
		...
		$results = $this->model_catalog_review_ext->getReviews($filter_data);

		$review_total = $this->model_catalog_review_ext->getTotalReviews();
				...
				switch ($action) {
					case 'edit':
						$_buttons[] = array(
							...
							'url'   => html_entity_decode($this->url->link('catalog/review/edit', '&review_id=' . $result['review_id'] . '&token=' . $this->session->data['token'] . $url, 'SSL'), ENT_QUOTES, 'UTF-8'),
							'icon'  => 'pencil',
							'name'  => null,
		...
		$data['update_url'] = html_entity_decode($this->url->link('catalog/review/quick_update', 'token=' . $this->session->data['token'], 'SSL'));
		$data['refresh_url'] = html_entity_decode($this->url->link('catalog/review/refresh_data', 'token=' . $this->session->data['token'], 'SSL'));
		$data['load_popup_url'] = html_entity_decode($this->url->link('catalog/review/load_popup', 'token=' . $this->session->data['token'], 'SSL'));
		...
		$data['sorts'] = array();
		foreach ($this->config->get('aqe_catalog_reviews') as $column => $attr) {
			$data['sorts'][$column] = $this->url->link('catalog/review', 'token=' . $this->session->data['token'] . '&sort=' . $attr['sort'] . $url, 'SSL');
		}
		...
		$this->response->setOutput($this->load->view('catalog/aqe/review_ext_list.tpl', $data));
	}
	...
	public function quick_update() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->validateUpdateData($this->request->post)) {
			...
			foreach ((array)$id as $_id) {
				if ($this->model_catalog_review_ext->quickEditReview($_id, $column, $value, $lang_id, $this->request->post)) {
					$results['done'][] = $_id;
				} else {
					$results['failed'][] = $_id;
				}
	...
	protected function validateUpdateData(&$data) {
		...
		if ($column == "rating") {
			if (!isset($data['new']) || $data['new'] < 0 || $data['new'] > 5) {
				$errors = true;
