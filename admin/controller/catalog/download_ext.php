...
class ControllerCatalogDownloadExt extends Controller {
	...
	public function __construct($registry) {
		...
		if (!$this->config->get('aqe_installed') || !$this->config->get('aqe_status')) {
			$this->response->redirect($this->url->link('catalog/download', 'token=' . $this->session->data['token'], 'SSL'));
		}
	}
	...
	protected function getList() {
		...
		$download_total = $this->model_catalog_download_ext->getTotalDownloads();
				...
				switch ($action) {
					case 'edit':
						$_buttons[] = array(
							...
							'url'   => html_entity_decode($this->url->link('catalog/download/edit', '&download_id=' . $result['download_id'] . '&token=' . $this->session->data['token'] . $url, 'SSL'), ENT_QUOTES, 'UTF-8'),
							'icon'  => 'pencil',
							'name'  => null,
		...
		$data['update_url'] = html_entity_decode($this->url->link('catalog/download/quick_update', 'token=' . $this->session->data['token'], 'SSL'));
		$data['load_popup_url'] = html_entity_decode($this->url->link('catalog/download/load_popup', 'token=' . $this->session->data['token'], 'SSL'));
		...
		$data['sorts'] = array();
		foreach ($this->config->get('aqe_catalog_downloads') as $column => $attr) {
			$data['sorts'][$column] = $this->url->link('catalog/download', 'token=' . $this->session->data['token'] . '&sort=' . $attr['sort'] . $url, 'SSL');
		}
		...
		$this->response->setOutput($this->load->view('catalog/aqe/download_ext_list.tpl', $data));
	}
	...
	public function quick_update() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->validateUpdateData($this->request->post)) {
			...
			foreach ((array)$id as $_id) {
				if ($this->model_catalog_download_ext->quickEditDownload($_id, $column, $value, $lang_id, $this->request->post)) {
					$results['done'][] = $_id;
				} else {
					$results['failed'][] = $_id;
				}
