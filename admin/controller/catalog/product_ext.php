...
class ControllerCatalogProductExt extends Controller {
	...
	protected function getList() {
				...
				switch ($action) {
					case 'edit':
						$_buttons[] = array(
							...
							'url'   => html_entity_decode($this->url->link('catalog/product/edit', '&product_id=' . $result['product_id'] . '&token=' . $this->session->data['token'] . $url, 'SSL'), ENT_QUOTES, 'UTF-8'),
							'icon'  => 'pencil',
							'name'  => null,
		...
		$data['update_url'] = html_entity_decode($this->url->link('catalog/product/quick_update', 'token=' . $this->session->data['token'], 'SSL'));
		$data['refresh_url'] = html_entity_decode($this->url->link('catalog/product/refresh_data', 'token=' . $this->session->data['token'], 'SSL'));
		...
		$data['load_popup_url'] = html_entity_decode($this->url->link('catalog/product/load_popup', 'token=' . $this->session->data['token'], 'SSL'));
		...
		$data['sorts'] = array();
		foreach ($this->config->get('aqe_catalog_products') as $column => $attr) {
			$data['sorts'][$column] = $this->url->link('catalog/product', 'token=' . $this->session->data['token'] . '&sort=' . $attr['sort'] . $url, 'SSL');
		}
		...
		$this->response->setOutput($this->load->view('catalog/aqe/product_ext_list.tpl', $data));
	}
	...
	public function load_popup() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->validateLoadPopup($this->request->post)) {
			...
			switch ($data['parameter']) {
				...
				case "seo":
					...
					$data['languages'] = $this->model_localisation_language->getLanguages();
					$data['value'] = array();
					$keywords = $this->model_catalog_product_ext->getProductSeoKeywords($data['product_id']);
		...
		$response['popup'] = $this->load->view('catalog/aqe/product_quick_form2.tpl', $data);
	...
	public function quick_update() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->validateUpdateData($this->request->post)) {
			...
			foreach ((array)$id as $_id) {
				if ($this->model_catalog_product_ext->quickEditProduct($_id, $column, $value, $lang_id, $this->request->post)) {
					$results['done'][] = $_id;
				} else {
					$results['failed'][] = $_id;
				}
			...
			if (isset($data['ids']) && count((array)$data['ids']) > 1) {
				...
				if (isset($data['value']) && $multilingual_seo) {
					foreach ((array)$data['value'] as $language_id => $value) {
						...
						if ($this->model_catalog_product_ext->urlAliasExists($id, $keyword, $language_id)) {
							$errors = true;
