<?php
class ControllerCatalogOptionExt extends Controller {
	...
	public function __construct($registry) {
		...
		$this->load->model('catalog/option_ext');

		$this->load->language('catalog/option');
		$this->load->language('catalog/qe_general');
		$this->load->language('catalog/option_ext');

		if (!$this->config->get('aqe_installed') || !$this->config->get('aqe_status')) {
			$this->response->redirect($this->url->link('catalog/option', 'token=' . $this->session->data['token'], 'SSL'));
		}
	}
	...
	protected function getList() {
		...
		$results = $this->model_catalog_option_ext->getOptions($filter_data);

		$option_total = $this->model_catalog_option_ext->getTotalOptions();
		...
		foreach ($results as $result) {
				...
				switch ($action) {
					case 'edit':
						$_buttons[] = array(
							...
							'url'   => html_entity_decode($this->url->link('catalog/option/edit', '&option_id=' . $result['option_id'] . '&token=' . $this->session->data['token'] . $url, 'SSL'), ENT_QUOTES, 'UTF-8'),
							'icon'  => 'pencil',
							'name'  => null,
			...
			if (!is_array($columns)) {
				...
				foreach ($columns as $column => $attr) {
					// if ($column == 'option') {
					// 	$fs = $this->model_catalog_option_ext->getOptionsByOptionGroupId($result['option_id']);
					// 	$options = array();
		...
		$data['update_url'] = html_entity_decode($this->url->link('catalog/option/quick_update', 'token=' . $this->session->data['token'], 'SSL'));
		$data['refresh_url'] = html_entity_decode($this->url->link('catalog/option/refresh_data', 'token=' . $this->session->data['token'], 'SSL'));
		...
		$data['load_popup_url'] = html_entity_decode($this->url->link('catalog/option/load_popup', 'token=' . $this->session->data['token'], 'SSL'));
		...
		$data['sorts'] = array();
		foreach ($this->config->get('aqe_catalog_options') as $column => $attr) {
			$data['sorts'][$column] = $this->url->link('catalog/option', 'token=' . $this->session->data['token'] . '&sort=' . $attr['sort'] . $url, 'SSL');
		}
		...
		$this->response->setOutput($this->load->view('catalog/aqe/option_ext_list.tpl', $data));
	}
	...
	public function autocomplete2() {
		...
		if (isset($this->request->get['filter_name'])) {
			...
			$results = $this->model_catalog_option_ext->getOptionValues($filter_data);
	...
	public function quick_update() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->validateUpdateData($this->request->post)) {
			...
			foreach ((array)$id as $_id) {
				if ($this->model_catalog_option_ext->quickEditOption($_id, $column, $value, $lang_id, $this->request->post)) {
					$results['done'][] = $_id;
				} else {
					$results['failed'][] = $_id;
				}
