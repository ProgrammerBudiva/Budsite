<?php
class ControllerCatalogAqeDownload extends Controller {
	...
	public function __construct($registry) {
		...
		$this->load->model('catalog/aqe/download');

		$this->load->language('catalog/download');
		$this->load->language('catalog/aqe/general');
		$this->load->language('catalog/aqe/download');

		if (!$this->config->get('aqe_installed') || !$this->config->get('aqe_status')) {
			$this->response->redirect($this->url->link('catalog/download', 'token=' . $this->session->data['token'], true));
		}
	}
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $item_id) {
				$this->model_catalog_download->deleteDownload($item_id);
			}
	...
	protected function getList() {
		...
		$data['download_actions'] = $actions;

		$columns = $this->config->get('aqe_catalog_downloads');
		...
		$results = $this->model_catalog_aqe_download->getDownloads($filter_data);

		$download_total = $this->model_catalog_aqe_download->getTotalDownloads();
		...
		foreach ($results as $result) {
				...
				switch ($action) {
					case 'edit':
						$_buttons[] = array(
							...
							'url'   => html_entity_decode($this->url->link('catalog/download/edit', '&download_id=' . $result['download_id'] . '&token=' . $this->session->data['token'] . $url, true), ENT_QUOTES, 'UTF-8'),
							'icon'  => 'pencil',
							'name'  => null,
			...
			if (!is_array($columns)) {
				...
				foreach ($columns as $column => $attr) {
					if ($column == 'id') {
						$row[$column] = $result['download_id'];
					} else if ($column == 'action') {
						$row[$column] = $_buttons;
					} else if ($column == 'date_added') {
						// $row[$column] = date($this->language->get('date_format_short'), strtotime($result['date_added']));
		...
		if (!is_array($columns)) {
			$displayed_columns = array('selector', 'name', 'date_added', 'action');
			$columns = array();
		} else {
			foreach ($columns as $column => $attr) {
				if (empty($attr['sort'])) {
		...
		$data['update_url'] = html_entity_decode($this->url->link('catalog/download/quick_update', 'token=' . $this->session->data['token'], true));
		$data['load_popup_url'] = html_entity_decode($this->url->link('catalog/download/load_popup', 'token=' . $this->session->data['token'], true));
		...
		$data['sorts'] = array();
		foreach ($this->config->get('aqe_catalog_downloads') as $column => $attr) {
			$data['sorts'][$column] = $this->url->link('catalog/download', 'token=' . $this->session->data['token'] . '&sort=' . $attr['sort'] . $url, true);
		}
		...
		if (version_compare(VERSION, '2.2.0', '>=')) {
			$template = 'catalog/aqe/download_list';
		} else {
			$template = 'catalog/aqe/download_list.tpl';
		}
	...
	public function load_popup() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->validateLoadPopup($this->request->post)) {
			...
			switch ($data['parameter']) {
				case "name":
					...
					$data['value'] = array();
					$descriptions = $this->model_catalog_download->getDownloadDescriptions($data['i_id']);
		...
		if (version_compare(VERSION, '2.2.0', '>=')) {
			...
			$template = 'catalog/aqe/quick_edit_form2.tpl';
		}
	...
	public function quick_update() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->validateUpdateData($this->request->post)) {
			...
			foreach ((array)$id as $_id) {
				if ($this->model_catalog_aqe_download->quickEditDownload($_id, $column, $value, $lang_id, $this->request->post)) {
					$results['done'][] = $_id;
				} else {
					$results['failed'][] = $_id;
				}
	...
	protected function validateDelete() {
		...
		foreach ($this->request->post['selected'] as $download_id) {
			...
			if ($product_total) {
				$error = true;
				$d = $this->model_catalog_download->getDownloadDescriptions($download_id);
				$name = isset($d[$this->config->get('config_language_id')]['name']) ? $d[$this->config->get('config_language_id')]['name'] : '';
				$this->alert['warning']['delete' . $download_id] = sprintf($this->language->get('error_delete'), $name, $product_total);
			}
		}
	...
	protected function validateUpdateData(&$data) {
		...
		if ($column == "mask") {
			if ((utf8_strlen($data['new']) < 3) || (utf8_strlen($data['new']) > 128)) {
				$errors = true;
				$this->alert['error']['mask'] = $this->language->get('error_mask');
			}
		}
