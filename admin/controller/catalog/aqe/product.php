...
class ControllerCatalogAqeProduct extends Controller {
	...
	protected function getList() {
		...
		$data['column_requires_shipping'] = $this->language->get('column_requires_shipping');
			...
			if (!is_array($columns)) {
				...
				foreach ($columns as $column => $attr) {
					if ($column == 'image') {
						...
						$row['requires_shipping'] = ($result['shipping'] ? $this->language->get('text_yes') : $this->language->get('text_no'));
	...
	public function load_popup() {
		...
		if (version_compare(VERSION, '2.2.0', '>=')) {
			...
			$template = 'catalog/aqe/product_quick_form2.tpl';
		}
