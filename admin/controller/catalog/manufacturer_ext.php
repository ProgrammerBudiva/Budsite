<?php
class ControllerCatalogManufacturerExt extends Controller {
	...
	public function __construct($registry) {
		...
		$this->load->model('catalog/manufacturer_ext');

		$this->load->language('catalog/manufacturer');
		$this->load->language('catalog/qe_general');
		$this->load->language('catalog/manufacturer_ext');
	...
	protected function getList() {
		...
		$data['rebuild'] = $this->url->link('catalog/manufacturer/repair', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		$results = $this->model_catalog_manufacturer_ext->getManufacturers($filter_data);

		$manufacturer_total = $this->model_catalog_manufacturer_ext->getTotalManufacturers();
				...
				switch ($action) {
					case 'edit':
						$_buttons[] = array(
							...
							'url'   => html_entity_decode($this->url->link('catalog/manufacturer/edit', '&manufacturer_id=' . $result['manufacturer_id'] . '&token=' . $this->session->data['token'] . $url, 'SSL'), ENT_QUOTES, 'UTF-8'),
							'icon'  => 'pencil',
							'name'  => null,
		...
		$data['update_url'] = html_entity_decode($this->url->link('catalog/manufacturer/quick_update', 'token=' . $this->session->data['token'], 'SSL'));
		$data['refresh_url'] = html_entity_decode($this->url->link('catalog/manufacturer/refresh_data', 'token=' . $this->session->data['token'], 'SSL'));
		$data['load_popup_url'] = html_entity_decode($this->url->link('catalog/manufacturer/load_popup', 'token=' . $this->session->data['token'], 'SSL'));
		...
		$data['sorts'] = array();
		foreach ($this->config->get('aqe_catalog_manufacturers') as $column => $attr) {
			$data['sorts'][$column] = $this->url->link('catalog/manufacturer', 'token=' . $this->session->data['token'] . '&sort=' . $attr['sort'] . $url, 'SSL');
		}
		...
		$this->response->setOutput($this->load->view('catalog/aqe/manufacturer_ext_list.tpl', $data));
	}
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->validateLoadPopup($this->request->post)) {
			...
			switch ($data['parameter']) {
				case "seo":
					...
					$data['value'] = array();
					$keywords = $this->model_catalog_manufacturer_ext->getManufacturerSeoKeywords($data['i_id']);
	...
	public function quick_update() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->validateUpdateData($this->request->post)) {
			...
			foreach ((array)$id as $_id) {
				if ($this->model_catalog_manufacturer_ext->quickEditManufacturer($_id, $column, $value, $lang_id, $this->request->post)) {
					$results['done'][] = $_id;
				} else {
					$results['failed'][] = $_id;
				}
			...
			if (isset($data['ids']) && count((array)$data['ids']) > 1) {
				...
				if (isset($data['value']) && $multilingual_seo) {
					foreach ((array)$data['value'] as $language_id => $value) {
						...
						if ($this->model_catalog_manufacturer_ext->urlAliasExists($id, $keyword, $language_id)) {
							$errors = true;
					...
					if ($this->model_catalog_manufacturer_ext->urlAliasExists($id, $keyword)) {
						$errors = true;
