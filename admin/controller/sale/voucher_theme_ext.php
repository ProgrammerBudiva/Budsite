<?php
class ControllerSaleVoucherThemeExt extends Controller {
	...
	public function __construct($registry) {
		...
		$this->load->model('sale/voucher_theme_ext');

		$this->load->language('sale/voucher_theme');
		$this->load->language('sale/qe_general');
		$this->load->language('sale/voucher_theme_ext');

		if (!$this->config->get('aqe_installed') || !$this->config->get('aqe_status')) {
			$this->response->redirect($this->url->link('sale/voucher_theme', 'token=' . $this->session->data['token'], 'SSL'));
		}
	}
	...
	protected function getList() {
		...
		$results = $this->model_sale_voucher_theme_ext->getVoucherThemes($filter_data);

		$voucher_theme_total = $this->model_sale_voucher_theme_ext->getTotalVoucherThemes();
				...
				switch ($action) {
					case 'edit':
						$_buttons[] = array(
							...
							'url'   => html_entity_decode($this->url->link('sale/voucher_theme/edit', '&voucher_theme_id=' . $result['voucher_theme_id'] . '&token=' . $this->session->data['token'] . $url, 'SSL'), ENT_QUOTES, 'UTF-8'),
							'icon'  => $attr['icon'],
							'name'  => null,
		...
		$data['update_url'] = html_entity_decode($this->url->link('sale/voucher_theme/quick_update', 'token=' . $this->session->data['token'], 'SSL'));
		$data['load_popup_url'] = html_entity_decode($this->url->link('sale/voucher_theme/load_popup', 'token=' . $this->session->data['token'], 'SSL'));
		...
		$data['sorts'] = array();
		foreach ($this->config->get('aqe_sales_voucher_themes') as $column => $attr) {
			$data['sorts'][$column] = $this->url->link('sale/voucher_theme', 'token=' . $this->session->data['token'] . '&sort=' . $attr['sort'] . $url, 'SSL');
		}
		...
		$this->response->setOutput($this->load->view('sale/aqe/voucher_theme_ext_list.tpl', $data));
	}
	...
	public function quick_update() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->validateUpdateData($this->request->post)) {
			...
			foreach ((array)$id as $_id) {
				if ($this->model_sale_voucher_theme_ext->quickEditVoucherTheme($_id, $column, $value, $lang_id, $this->request->post)) {
					$results['done'][] = $_id;
				} else {
					$results['failed'][] = $_id;
				}
