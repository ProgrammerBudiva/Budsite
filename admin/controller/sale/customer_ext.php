<?php
class ControllerSaleCustomerExt extends Controller {
	...
	public function __construct($registry) {
		...
		$this->load->model('sale/customer_ext');

		$this->load->language('sale/customer');
		$this->load->language('sale/qe_general');
		$this->load->language('sale/customer_ext');
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $item_id) {
				$this->model_sale_customer->deleteCustomer($item_id);
			}
	...
	protected function getList() {
		...
		$columns = $this->config->get('aqe_sales_customers');
		...
		$results = $this->model_sale_customer_ext->getCustomers($filter_data);

		$customer_total = $this->model_sale_customer_ext->getTotalCustomers();
				...
				switch ($action) {
					case 'edit':
						$_buttons[] = array(
							...
							'url'   => html_entity_decode($this->url->link('sale/customer/edit', '&customer_id=' . $result['customer_id'] . '&token=' . $this->session->data['token'] . $url, 'SSL'), ENT_QUOTES, 'UTF-8'),
							'icon'  => $attr['icon'],
							'name'  => null,
					...
					case 'approve':
						$_buttons[] = array(
							...
							'action'=> $action,
							'title' => $this->language->get('action_' . $action),
							'url'   => !$result['approved'] ? html_entity_decode($this->url->link('sale/customer/approve', '&customer_id=' . $result['customer_id'] . '&token=' . $this->session->data['token'] . $url, 'SSL'), ENT_QUOTES, 'UTF-8') : '',
							'icon'  => $attr['icon'],
							'name'  => null,
							'rel'   => json_encode($attr['rel']),
							'class' => $attr['class'],
					...
					case 'unlock':
						...
						$_buttons[] = array(
							...
							'action'=> $action,
							'title' => $this->language->get('action_' . $action),
							'url'   => ($login_info && $login_info['total'] >= $this->config->get('config_login_attempts')) ? html_entity_decode($this->url->link('sale/customer/unlock', '&email=' . $result['email'] . '&token=' . $this->session->data['token'] . $url, 'SSL'), ENT_QUOTES, 'UTF-8') : '',
							'icon'  => $attr['icon'],
							'name'  => null,
							'rel'   => json_encode($attr['rel']),
							'class' => $attr['class'],
		...
		$data['update_url'] = html_entity_decode($this->url->link('sale/customer/quick_update', 'token=' . $this->session->data['token'], 'SSL'));
		$data['load_popup_url'] = html_entity_decode($this->url->link('sale/customer/load_popup', 'token=' . $this->session->data['token'], 'SSL'));
		...
		$data['sorts'] = array();
		foreach ($this->config->get('aqe_sales_customers') as $column => $attr) {
			$data['sorts'][$column] = $this->url->link('sale/customer', 'token=' . $this->session->data['token'] . '&sort=' . $attr['sort'] . $url, 'SSL');
		}
		...
		$this->response->setOutput($this->load->view('sale/aqe/customer_ext_list.tpl', $data));
	}
	...
	public function load_popup() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->validateLoadPopup($this->request->post)) {
			...
			switch ($data['parameter']) {
				case "name":
					...
					$return = $this->model_sale_customer->getCustomer($data['i_id']);
		...
		$response['popup'] = $this->load->view('sale/aqe/quick_edit_form2.tpl', $data);
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->validateUpdateData($this->request->post)) {
			...
			if ($column == 'approve') {
				foreach ((array)$id as $_id) {
					$this->model_sale_customer->approve($_id);
					$results['done'][] = $_id;
				}
			} else {
				foreach ((array)$id as $_id) {
					if ($this->model_sale_customer_ext->quickEditCustomer($_id, $column, $value, $lang_id, $this->request->post)) {
						$results['done'][] = $_id;
					} else {
						$results['failed'][] = $_id;
					}
	...
	protected function validateUpdateData(&$data) {
		...
		if ($column == "email") {
			$customer_info = $this->model_sale_customer->getCustomerByEmail($data['new']);
