<?php
class ControllerSaleReturnExt extends Controller {
	...
	public function __construct($registry) {
		...
		$this->load->model('sale/return_ext');

		$this->load->language('sale/return');
		$this->load->language('sale/qe_general');
		$this->load->language('sale/return_ext');

		if (!$this->config->get('aqe_installed') || !$this->config->get('aqe_status')) {
			$this->response->redirect($this->url->link('sale/return', 'token=' . $this->session->data['token'], 'SSL'));
		}
	}
	...
	protected function getList() {
		...
		$results = $this->model_sale_return_ext->getReturns($filter_data);

		$return_total = $this->model_sale_return_ext->getTotalReturns();
				...
				switch ($action) {
					case 'edit':
						$_buttons[] = array(
							...
							'url'   => html_entity_decode($this->url->link('sale/return/edit', '&return_id=' . $result['return_id'] . '&token=' . $this->session->data['token'] . $url, 'SSL'), ENT_QUOTES, 'UTF-8'),
							'icon'  => 'pencil',
							'name'  => null,
			...
			if (!is_array($columns)) {
				...
				foreach ($columns as $column => $attr) {
					...
					// 	}
						...
						$row['product_href'] = $this->url->link('catalog/product/update', 'token=' . $this->session->data['token'] . '&product_id=' . $result['product_id'], 'SSL');
					} else if ($column == 'customer_id') {
						// $row[$column] = $result[$column];
						$row[$column] = $result['customer_name'];
						$row['product_href'] = $this->url->link('sale/customer/update', 'token=' . $this->session->data['token'] . '&customer_id=' . $result['customer_id'], 'SSL');
		...
		$data['update_url'] = html_entity_decode($this->url->link('sale/return/quick_update', 'token=' . $this->session->data['token'], 'SSL'));
		$data['refresh_url'] = html_entity_decode($this->url->link('sale/return/refresh_data', 'token=' . $this->session->data['token'], 'SSL'));
		...
		$data['load_popup_url'] = html_entity_decode($this->url->link('sale/return/load_popup', 'token=' . $this->session->data['token'], 'SSL'));
		...
		$data['sorts'] = array();
		foreach ($this->config->get('aqe_sales_returns') as $column => $attr) {
			$data['sorts'][$column] = $this->url->link('sale/return', 'token=' . $this->session->data['token'] . '&sort=' . $attr['sort'] . $url, 'SSL');
		}
		...
		$this->response->setOutput($this->load->view('sale/aqe/return_ext_list.tpl', $data));
	}
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->validateUpdateData($this->request->post)) {
			...
			if ($column == 'return_status') {
				...
				foreach ((array)$id as $_id) {
					if ($this->model_sale_return_ext->quickEditReturn($_id, $column, $value, $lang_id, $this->request->post)) {
						$results['done'][] = $_id;
					} else {
						$results['failed'][] = $_id;
					}
