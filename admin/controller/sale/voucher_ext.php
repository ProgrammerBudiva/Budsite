<?php
class ControllerSaleVoucherExt extends Controller {
	...
	public function __construct($registry) {
		...
		$this->load->model('sale/voucher_ext');

		$this->load->language('sale/voucher');
		$this->load->language('sale/qe_general');
		$this->load->language('sale/voucher_ext');

		if (!$this->config->get('aqe_installed') || !$this->config->get('aqe_status')) {
			$this->response->redirect($this->url->link('sale/voucher', 'token=' . $this->session->data['token'], 'SSL'));
		}
	}
	...
	protected function getList() {
		...
		$data['send'] = $this->url->link('sale/voucher/send', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		$results = $this->model_sale_voucher_ext->getVouchers($filter_data);

		$voucher_total = $this->model_sale_voucher_ext->getTotalVouchers();
				...
				switch ($action) {
					case 'edit':
						$_buttons[] = array(
							...
							'url'   => html_entity_decode($this->url->link('sale/voucher/edit', '&voucher_id=' . $result['voucher_id'] . '&token=' . $this->session->data['token'] . $url, 'SSL'), ENT_QUOTES, 'UTF-8'),
							'icon'  => $attr['icon'],
							'name'  => null,
					...
					case 'send':
						$_buttons[] = array(
							...
							'action'=> $action,
							'title' => $this->language->get('action_' . $action),
							'url'   => html_entity_decode($this->url->link('sale/voucher/send', '&voucher_id=' . $result['voucher_id'] . '&token=' . $this->session->data['token'], 'SSL'), ENT_QUOTES, 'UTF-8'),
							'icon'  => $attr['icon'],
							'name'  => null,
							'rel'   => json_encode($attr['rel']),
							'class' => $attr['class'],
		...
		$data['update_url'] = html_entity_decode($this->url->link('sale/voucher/quick_update', 'token=' . $this->session->data['token'], 'SSL'));
		...
		$data['sorts'] = array();
		foreach ($this->config->get('aqe_sales_vouchers') as $column => $attr) {
			$data['sorts'][$column] = $this->url->link('sale/voucher', 'token=' . $this->session->data['token'] . '&sort=' . $attr['sort'] . $url, 'SSL');
		}
		...
		$this->response->setOutput($this->load->view('sale/aqe/voucher_ext_list.tpl', $data));
	}
	...
	public function quick_update() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->validateUpdateData($this->request->post)) {
			...
			foreach ((array)$id as $_id) {
				if ($this->model_sale_voucher_ext->quickEditVoucher($_id, $column, $value, $lang_id, $this->request->post)) {
					$results['done'][] = $_id;
				} else {
					$results['failed'][] = $_id;
				}
