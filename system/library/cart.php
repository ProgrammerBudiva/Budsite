...
class Cart {
			...
			foreach ($this->session->data['cart'] as $key => $quantity) {
				...
				if ($product_query->num_rows) {
					...
					$this->data[$key] = array(
						...
						'special_price'   => ($special_price)?($special_price + $option_price):false,
                        'price_discount'  => ($special_price)?((1 - ($special_price/$price))*100):false,
						'total'           => ($special_price)?($special_price + $option_price) * $quantity:($price + $option_price) * $quantity,
	...
	public function getTotal() {
		...
		foreach ($this->getProducts() as $product) {
			$total += $this->tax->calculate($product['special_price']?$product['special_price']:$product['price'], $product['tax_class_id'], $this->config->get('config_tax')) * $product['quantity'];
		}
