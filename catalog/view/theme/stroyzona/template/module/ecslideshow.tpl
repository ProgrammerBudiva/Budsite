<div class="ecslideshow <?php echo (isset($support_rtl) && $support_rtl)?'rtl':'';?>" style="width:<?php echo $module_width;?>;height:<?php echo $module_height;?>">
	<div class="camera_wrap <?php echo $skin;?>" id="ecslideshow<?php echo $module;?>">
		...
		if(isset($slides) && !empty($slides)){
          ...
          <meta data-type="slide-info" data-slide-link="<?= $slide['link']?>" content='<?= json_encode(['name'=>$slide['caption'], 'position' => $index], JSON_UNESCAPED_UNICODE)?>'/>
 					<!--
...
<script type="text/javascript">
  ...
  $(function(){
    ...
    /* Событие promoView в слайдере главной страницы */
    var promotions = [];
    ...
    $('#ecslideshow0').on('click', '.camera_link', function(e){
      ...
      var promo = JSON.parse($('[data-slide-link="' + link + '"]').attr('content'));
