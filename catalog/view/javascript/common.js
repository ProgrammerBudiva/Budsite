    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $.each($.parseJSON(data), function (index, value) {
                ...
                /*$('select#input-point-new-post').append(
...
$(document).ready(function () {
    ...
    /*$('.delivery-pay-popup').load('index.php?route=information/information/agree&information_id=19')*/
    ...
    $('.user-panel').hover(function () {
        if ($(this).hasClass('first-login')) {
            $.ajax({
                url: 'index.php?route=account/login/new_login',
                success: function (data) {
    ...
    /*$('.to-drop, .link-close-btn').click(function () {
        $('.my-dropdown').toggle();
        $('.gray-bg-all').toggle();
        $('.user-forms-login').show();
    });*/
    ...
    /*$(document).click(function (event) {
        ...
        if ($(ET).closest("#top-links").length) return;
        $('.my-dropdown').hide();
        $('.gray-bg-all').hide();
...
var cart = {
    ...
    'remove': function (key, product) {
        /* Событие removeFromCart для Enhanced ecommerce */
        ...
        $.ajax({
            ...
            success: function (json) {
                ...
                if (json['success']) {
                    ...
                    /*let isProductPage = $(el).closest('.product-to-do').length > 0 ? true:false;
                    if(isProductPage) {
                        $(el).notify(
                            {
                                text: json['success']
...
*/
                    /*if(typeof(el) != 'undefined' && $(window).width() > 768) {
                        const offset_right = $(window).width() - $(el).offset().left + $(el).width();
                        let pos = offset_right < 380 ? 'left top':'right top';
                        const container = $(el).closest('.product-thumb').parent('div');
                        const parent_container = container.closest('.slick-track');
                        const index = parent_container.find('.slick-active').index(container);
...
$.notify.addStyle('success', {
    html:
    '<div class="alert alert-success"><i class="fa fa-check-circle" style="margin-right:4px"></i>&nbsp;<p style="display:inline" data-notify-html="text"></p></div>',
    classes: {
        styled: {
...
$.notify.addStyle('error', {
    html:
        '<div class="alert alert-success"><i class="fa fa-info-circle" style="margin-right:4px"></i>&nbsp;<p style="display:inline" data-notify-html="text"></p></div>',
    classes: {
        styled: {
            "background-color": "#ffe5d5",
            "border": "2px solid #f6b283",
