 ...
 * Time: 10:54
 */
class ControllerPricespdfTeploizolyaciya extends Controller
{
    public function index()
    {
        ...
        $query = $this->db->query("SELECT tagged_related FROM " . DB_PREFIX ."category WHERE category_id='211'"  );
        $subcat = explode(',',   $query->row['tagged_related']);

        $data = ['filter_category_id' => 211 ];
        $results[] = $this->model_catalog_product->getProducts($data);
        foreach ($subcat as $value){

            $results[] = $this->model_catalog_product->getProducts(['filter_category_id' => $value]);
        }
