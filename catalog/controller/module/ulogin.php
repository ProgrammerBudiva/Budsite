...
class ControllerModuleUlogin extends Controller {
    ...
    public function index($setting) {
        if ($this->customer->isLogged() && !empty($this->request->post)) {
        ...
        if (isset($this->session->data['ulogin_token']) && !$this->customer->isLogged()) {
    ...
    public function login($token) {
        ...
        $key = (isset($token['token']))?$token['token']:'';
        $s = file_get_contents('https://ulogin.ru/token.php?token=' . $key . '&host=' . $domain);
        ...
        if (isset($user['email'])) {
            if ($this->customer->login($user['email'], '', true)) {
                ...
                $this->model_account_customer->ajaxAddCustomer($data);
