...
//		}
    ...
    $options_data = array();
    /* вспомогательный массив для сбора массива вида $option['option_id'] => $key из $options */
    $option_id_to_key = array();

		if ($options) {
		  foreach ($options as $key => $option) { # Start options each
        $this_option = isset($this->options_get[$option['option_id']]);

        $option_id_to_key[$option['option_id']] = $key;
        ...
        /* Функция проверки что опция или имеет выбранные значения или содержит 2 и более значения */
        $hideOptionByValuesCount = function() use($values, $this_option){
...
//        }
    ...
    /* Функция-замыкание для сортировки опций специфическим для магазина способом */
    $sortOptionValues = function ($option_data) {
      if (!empty($option_data['values'])) {
        usort($option_data['values'], function($a, $b){
          $v1 = $a['name'];
          $v2 = $b['name'];

          // функция сравнения числовых значений
          $numericCompare = function($v1, $v2){
            if ($v1 == $v2) {
              return 0;
            }
            return ($v1 > $v2) ? 1 : -1;
          ...
          // если нормальные числа
          if (is_numeric($v1) && is_numeric($v2)) {
            return $numericCompare($v1, $v2);
          }

          // если строка вида "7,5 см х 10м", то сравниваем по числу, которое получим из части строки до первого пробела
          /* Функция, которая пытается получить число с точкой из части строки до первого пробела */
          $getFirstWord = function ($str) {
            preg_match('/^[\d\.,]+\s/ui', $str, $matches);
            $word = !empty($matches) ? trim($matches[0]) : FALSE;
            if ($word) {
          ...
          $firstWord1 = $getFirstWord($v1);
          $firstWord2 = $getFirstWord($v2);
          if (is_numeric($firstWord1) && is_numeric($firstWord2)) {
            return $numericCompare($firstWord1, $firstWord2);
          }

          // по умолчанию сравниваем как строки
          return strcmp(mb_strtoupper($v1), mb_strtoupper($v2));
        });
      }
    ...
    foreach ($options_data as &$option_data) {
      // сортировка значений опции
      $option_data['values'] = $sortOptionValues($option_data);

      // добавление к значениям опций флага открытости для индексации
      $key = $option_id_to_key[$option_data['option_id']];
      $option_data['open_index'] = !empty($options[$key]['open_index']) ? $options[$key]['open_index'] : 0;
    }

    // Выведем сами опции по алфавиту
