...
class ControllerFeedYandexMarket extends Controller {
  ...
  private $eol = "\n";
  const BRAND_ATTR_ID = 2;

  public function index($params = []) {
    if ($this->config->get('yandex_market_status')) {

      if (!($allowed_categories = $this->config->get('yandex_market_categories'))) {
        exit();
      }
      ...
      foreach ($currencies as $currency) {
        if ($currency['code'] != $offers_currency && $currency['status'] == 1) {
          $this->setCurrency(
            $currency['code'],
            number_format(1 / $this->currency->convert($currency['value'], $offers_currency, $shop_currency), 4, '.', '')
          );
        }
      }
      ...
      foreach ($products as $product) {
        ...
        $data['url'] =
          $this->url->link('product/product', 'path=' . $this->getPath($product['category_id']) . '&product_id=' . $product['product_id']);

        // Price
        $special = !empty($product['special']) ? $product['special'] : false;
        $price = !empty($product['price']) ? $product['price'] : 0;;
        if ($special && $special < $price) {
          $data['price'] = number_format(
            $this->currency->convert($this->tax->calculate($special, $product['tax_class_id']), $shop_currency, $offers_currency),
            $decimal_place,
            '.',
            ''
          );

          $data['oldprice'] = number_format(
            $this->currency->convert($this->tax->calculate($price, $product['tax_class_id']), $shop_currency, $offers_currency),
            $decimal_place,
            '.',
            ''
          );
        ...
        if (empty($params['provider']) || $params['provider'] != 'ibud') {
  ...
  public function ibud(){
    $this->index(['provider' => 'ibud']);
  }
  ...
  private function setOffer($data) {
    ...
    switch ($type) {
      case 'vendor.model':
        $allowed_tags =
          array_merge($allowed_tags, array('typePrefix' => 0, 'vendor' => 1, 'vendorCode' => 0, 'model' => 1, 'provider' => 0, 'tarifplan' => 0));
        break;
      ...
      case 'artist.title':
        $allowed_tags = array_merge(
          $allowed_tags,
          array('artist' => 0, 'title' => 1, 'year' => 0, 'media' => 0, 'starring' => 0, 'director' => 0, 'originalName' => 0, 'country' => 0)
        );
        break;
      ...
      case 'event-ticket':
        $allowed_tags = array_merge(
          $allowed_tags,
          array('name' => 1, 'place' => 1, 'hall' => 0, 'hall_part' => 0, 'date' => 1, 'is_premiere' => 0, 'is_kids' => 0)
        );
        break;
