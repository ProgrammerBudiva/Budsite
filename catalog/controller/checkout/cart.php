...
class ControllerCheckoutCart extends Controller {
  ...
  public function index() {
    ...
    if ($this->cart->hasProducts() || !empty($this->session->data['vouchers'])) {
      ...
      $data['column_model_size'] = $this->language->get('column_model_size');
      ...
      $data['button_refresh_total'] = $this->language->get('button_refresh_total');
      ...
      $data['action_cart'] = $this->url->link('checkout/cart/ajax_edit');
      ...
      foreach ($products as $product) {
        ...
        if ($product['special_price']) {
          $total = $this->currency->format(
            $this->tax->calculate($product['special_price'], $product['tax_class_id'], $this->config->get('config_tax')) * $product['quantity']
          );
        ...
        // get description (full info fetch, shit)
        ...
        $data['products'][] = array(
          ...
          'name'           => $product['name'],
          'model'          => $product['model'],
          'summary'        => current(explode(".", strip_tags(html_entity_decode($product_info['description'])))),
          'option'         => $option_data,
          'recurring'      => $recurring,
          'product_id'     => $product['product_id'],
          'original_price' => $product['special_price'] ? $product['special_price'] : $product['price'],
          'discount'       => ceil($product['price_discount']),
          ...
          'sizes'          => $product['length'] . 'x' . $product['width'] . 'x' . $product['height'],
          'brand'          => $this->model_catalog_product->getBrandFromAttributes($product['product_id']),
          'category'       => $this->model_catalog_product->getMainCategoryName($product['product_id']),
      ...
      if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/error/empty_cart.tpl')) {
        $this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/error/empty_cart.tpl', $data));
      } else {
        $this->response->setOutput($this->load->view('default/template/error/empty_cart.tpl', $data));
      }
    }
  }
...
//    echo "<pre>"; print_r($this->cart->getRealDiscount()); echo "</pre>";
    ...
    if (!empty($this->request->post['quantity'])) {
      ...
      if ($result) {
        ...
        foreach ($total_data as $key) {
          $json['total_' . $key['code']] = $this->currency->format($key['value']);
        }
        $json['total_count'] =
