...
class ModelCheckoutOrder extends Model {
    ...
    public function addNewOrder($data) {
        ...
        if (!empty($data['comment'])) {
          $this->db->query(
            "UPDATE `" . DB_PREFIX . "order` SET comment = '" . $this->db->escape($data['comment']) . "' WHERE order_id = '" . (int) $order_id . "'"
          );
        }
        ...
        $gtmDataLayer->transactionId             = $order_id;
        $gtmDataLayer->transactionAffiliation    = $data['store_name'];
        $gtmDataLayer->transactionTotal          = $data['total'];
        ...
        /* Объект отслеживания покупки для Enhanced Ecommerce
         * отличается от типвого формата для GTM отслеживания
         */
        $gtmDataLayer = new stdClass();
        $gtmDataLayer->event = "transactionComplete";
        $gtmDataLayer->ecommerce = new stdClass();
        $actionField = [
          'id'          => $order_id,
          'affiliation' => $data['store_name'],
          'revenue'     => $data['total'],
        ];
        $products = array_map(
          function ($product) {
            ...
            $obj->price = $product['price'];
            $obj->quantity = $product['quantity'];
            $obj->brand = $this->model_catalog_product->getBrandFromAttributes($product['product_id']);
            $obj->category = $this->model_catalog_product->getMainCategoryName($product['product_id']);
        ...
        // Send email with fucking shoputils module
        $this->load->model('module/shoputils_mail_order_status');
        $this->model_module_shoputils_mail_order_status->confirmOrder((int) $order_id, 1);
        $this->model_module_shoputils_mail_order_status->confirmOrderAdmin((int) $order_id, 1);
        // End Send email with fucking shoputils module
	...
	public function getOrder($order_id) {
		...
		if ($order_query->num_rows) {
			...
			return array(
				...
				'location'                => $order_query->row['location'],
