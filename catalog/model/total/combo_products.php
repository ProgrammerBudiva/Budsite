...
class ModelTotalComboProducts extends Model {
	public function getTotal(&$total_data, &$total, &$taxes) {
			...
			while ($this->check_exist_in_array($product_in_combo_id,$product_in_cart_id) && (count($product_in_combo_id) <= count($product_in_cart_id))) {
				if ($get_combo['discount_type'] == 'fixed amount')	{
					...
					foreach ($product_in_combo_id as $value){
						...
						if ($product_info['special']) $diff += $this->tax->calculate($product_info['price'], $product_info['tax_class_id'], $this->config->get('config_tax')) - $this->tax->calculate($product_info['special'], $product_info['tax_class_id'], $this->config->get('config_tax'));
					}
					if ($get_combo['override']) $amount += $get_combo['discount_number'] - $diff;
					else $amount += $get_combo['discount_number'];
				} else {
					$price = 0;
					$diff = 0;
					foreach ($product_in_combo_id as $value){
					...
					if ($get_combo['override']) $amount += $price*$get_combo['discount_number']/100 - $diff;
					else $amount += $price*$get_combo['discount_number']/100;				
				}		
		...
		if($amount) {
            $combo_name = array_unique($combo_name);
			$total_data[] = array(
				'code'       => 'combo_products',
	...
	public function check_exist_in_array($check,$array) {
		...
		if ($flag == count($check)) { $check_in = true;}
		return $check_in;	
	}
}	
?>