...
class ModelPaymentKaznachey extends Model {
	public function getMethod($address, $total = null) {
		...
		$total = $total ? $total : $_SESSION['total_cart_value'];
		...
		if ($this->config->get('kaznachey_total') > 0 && $this->config->get('kaznachey_total') > $total) {
			$status = false;
		} elseif (!$this->config->get('kaznachey_geo_zone_id')) {
			$status = true;
