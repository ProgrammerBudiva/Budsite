<?php
require '/home/budsit00/budsite.ua/www/export/msql.inc.php';
...
while ($row=mysql_fetch_assoc($orders))
{
    ...
    $paymentmethod=0;
    $paymentmethodname='';
    if(trim($row['shipping_method'])=='Курьерскими службами: Новая	Почта, Интайм, Mist Express') {$shipmethod=1;$shipmethodname='Курьерскими службами: Новая	Почта, Интайм, Mist Express';}
    if(trim($row['shipping_method'])=='Самовывоз') {$shipmethod=2;$shipmethodname='Самовывоз';};
    if(trim($row['shipping_method'])=='Адресная доставка по Одессе') {$shipmethod=3;$shipmethodname='Адресная доставка по Одессе';};

    if($row['payment_method']=='Наложенный  платеж (наличными при получении)') {$paymentmethod=1;$paymentmethodname='Наложенный  платеж (наличными при получении)';}
    if($row['payment_method']=='Наложенный	платеж (наличными курьеру, при получении)') {$paymentmethod=1;$paymentmethodname='Наложенный	платеж (наличными курьеру, при получении)';}
    if($row['payment_method']=='Наличными при получении в Одессе') {$paymentmethod=2;$paymentmethodname='Наличными при получении в Одессе';}
    if($row['payment_method']=='Кредитная карта Visa/MasterCard')
    {$paymentmethod=3;$paymentmethodname='Кредитная карта Visa/MasterCard';}
    if($row['payment_method']=='Безналичный расчет')
    {$paymentmethod=4;$paymentmethodname='Безналичный   расчет';}
    if(stripos($row['payment_method'],'Безналичный')!==false)
    {$paymentmethod=4;$paymentmethodname='Безналичный   расчет';}
    if($row['payment_method']=='Оплата на карту ПриватБанка')
    {$paymentmethod=5;$paymentmethodname='Оплата на карту ПриватБанка';}
    ...
    $out.='<shipmentmethod_name>'.trim($row['shipping_method']).'</shipmentmethod_name>'.PHP_EOL;
    $out.='</shipmentmethod>';
    $out.='<paymentmethod>';
    $out.='<paymentmethod_id>'.$paymentmethod.'</paymentmethod_id>'.PHP_EOL;
    $out.='<paymentmethod_name>'.$row['payment_method'].'</paymentmethod_name>'.PHP_EOL;
    $out.='</paymentmethod>';
    ...
    */
    ...
    while ($tv=mysql_fetch_assoc($tovars))
    {
        ...
        $oc_product2=mysql_fetch_assoc($oc_product);
        $tovarid=$oc_product2['tovarid'];
        //$tax=$tv['tax'];
        $out.= '<Товар>'.PHP_EOL;
...
$out.= '</clients>'.PHP_EOL;
*/
$file="/home/budsit00/budsite.ua/www/export/orderout_new.xml";
