...
$import=file_get_contents('/home/budsit00/budsite.com.ua/www/export/import.xml');
$offers=file_get_contents('/home/budsit00/budsite.com.ua/www/export/offers.xml');
/*
...
function find_attr_name($id,&$masiv)
{
    ...
    {
        if((string)$masiv->Классификатор->Свойства->Свойство[$n]->Ид==(string)$id)
        {
...
echo find_attr_name('7fa852ae-22cb-47bd-b3b9-cef0f92fdd47',$arr).'<br>';
$out='';
...
*/
    ...
    $out .= '<description>' . htmlspecialchars($arr->Каталог->Товары->Товар[$i]->Описание) . '</description>';
    $out .= '<name>' . $arr->Каталог->Товары->Товар[$i]->Наименование . '</name>';
    $image = $arr->Каталог->Товары->Товар[$i]->Картинка;
    $out .= '<image>' . $image . '</image>';
    if($arr->Каталог->Товары->Товар[$i]->ДополнительнаяКартинка1!='')
    {
        $out .= '<image1>'.$arr->Каталог->Товары->Товар[$i]->ДополнительнаяКартинка1.'</image1>';
    }
    if($arr->Каталог->Товары->Товар[$i]->ДополнительнаяКартинка2!='')
    {
        $out .= '<image2>'.$arr->Каталог->Товары->Товар[$i]->ДополнительнаяКартинка2.'</image2>';
    }
    if($arr->Каталог->Товары->Товар[$i]->ДополнительнаяКартинка3!='')
    {
        $out .= '<image3>'.$arr->Каталог->Товары->Товар[$i]->ДополнительнаяКартинка3.'</image3>';
    }
    if($arr->Каталог->Товары->Товар[$i]->ДополнительнаяКартинка4!='')
    {
        $out .= '<image4>'.$arr->Каталог->Товары->Товар[$i]->ДополнительнаяКартинка4.'</image4>';
    }
    if($arr->Каталог->Товары->Товар[$i]->ДополнительнаяКартинка5!='')
    {
        $out .= '<image5>'.$arr->Каталог->Товары->Товар[$i]->ДополнительнаяКартинка5.'</image5>';
    }
    if($arr->Каталог->Товары->Товар[$i]->ДополнительнаяКартинка6!='')
    {
        $out .= '<image6>'.$arr->Каталог->Товары->Товар[$i]->ДополнительнаяКартинка6.'</image6>';
    }
    if($arr->Каталог->Товары->Товар[$i]->ДополнительнаяКартинка7!='')
    {
        $out .= '<image7>'.$arr->Каталог->Товары->Товар[$i]->ДополнительнаяКартинка7.'</image7>';
    }
    if($arr->Каталог->Товары->Товар[$i]->ДополнительнаяКартинка8!='')
    {
        $out .= '<image8>'.$arr->Каталог->Товары->Товар[$i]->ДополнительнаяКартинка8.'</image8>';
    }
...
*/
    ...
    if($offernumber>=0)
    {
        ...
        $quantity=$arrof->ПакетПредложений->Предложения->Предложение[$offernumber]->Количество;
    }
    /*
    ...
    */
    ...
    $out.='<odyniciavymiry>'.$arr->Каталог->Товары->Товар[$i]->БазоваяЕдиница.'</odyniciavymiry>';
    $out.='</data>';
    $out.='<links>';
    ...
    $out.='<attributes>';
    $out.= '<attribute>';
    $out.= '<attrname>'.'Единица измерения'.'</attrname>';
    $out.= '<attrvalue>'.$arr->Каталог->Товары->Товар[$i]->БазоваяЕдиница.'</attrvalue>';
    $out.= '</attribute>';
        for($k=0;$k<count($arr->Каталог->Товары->Товар[$i]->ЗначенияСвойств->ЗначенияСвойства);$k++)
        {
            $attributename=find_attr_name((string)$arr->Каталог->Товары->Товар[$i]->ЗначенияСвойств->ЗначенияСвойства[$k]->Ид,$arr);
            if(!empty($attributename))
            {
            $out.= '<attribute>';
            $out.= '<attrname>'.$attributename.'</attrname>';
            $out.= '<attrvalue>'.trim($arr->Каталог->Товары->Товар[$i]->ЗначенияСвойств->ЗначенияСвойства[$k]->Значение).'</attrvalue>';
            $out.= '</attribute>';
            echo trim($arr->Каталог->Товары->Товар[$i]->ЗначенияСвойств->ЗначенияСвойства[$k]->Значение).'<br>';
            }
        }
    /*
    $out.= '<weight>'.$arr->Каталог->Товары->Товар[$i]->ЗначенияРеквизитов->ЗначениеРеквизита[3]->Значение.'</weight>';
...
$out.= '</products>';
$file='/home/budsit00/budsite.com.ua/www/export/openc_imp_auto.xml';
