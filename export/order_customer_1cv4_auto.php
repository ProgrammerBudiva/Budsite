...
while ($row=mysql_fetch_assoc($orders))
{
    ...
    $paymentmethod=0;
    $paymentmethodname='';
    if(trim($row['shipping_method'])=='Новая Почта') {$shipmethod=1;$shipmethodname='Новая Почта';}
    if(trim($row['shipping_method'])=='Самовывоз') {$shipmethod=2;$shipmethodname='Самовывоз';};

    if($row['payment_method']=='Оплата при доставке') {$paymentmethod=1;$paymentmethodname='Оплата при доставке';}
    if($row['payment_method']=='Банковский перевод') {$paymentmethod=2;$paymentmethodname='Банковский перевод';}
    if($row['payment_method']=='Кредитная карта Visa/MC, Webmoney, Liqpay, Qiwi... (www.kaznachey.ua)')
    {$paymentmethod=3;$paymentmethodname='Кредитная карта Visa/MC, Webmoney, Liqpay, Qiwi... (www.kaznachey.ua)';}
    ...
    $out.='<shipmentmethod_name>'.$shipmethodname.'</shipmentmethod_name>'.PHP_EOL;
    $out.='</shipmentmethod>';
    $out.='<paymentmethod>';
    $out.='<paymentmethod_id>'.$paymentmethod.'</paymentmethod_id>'.PHP_EOL;
    $out.='<paymentmethod_name>'.$paymentmethodname.'</paymentmethod_name>'.PHP_EOL;
    $out.='</paymentmethod>';
...
$out.= '</clients>'.PHP_EOL;
*/
$file="/home/budsit00/budsite.ua/www/export/orderout.xml";
