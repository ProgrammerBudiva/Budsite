...
*/
     ...
     $q27="INSERT INTO `oc_attribute_description` (`attribute_id`,`language_id`,`name`) VALUES ($aid,2,"."'".'Единица измерения'."'".")";
     mysql_query($q27) or die(mysql_error());

     $q3="INSERT INTO `oc_product_attribute` (`product_id`,`attribute_id`,`language_id`,`text`) VALUES($prdid,$aid,2,"."'".$arr3->product[$i]->data->odyniciavymiry."'".")";
     mysql_query($q3) or die(mysql_error());
     ...
     /*
       ...
       //(product_id,model,sku,upc,ean,jan,isbn,mpn,location,quantity,stock_status_id,image,manufacturer_id,shipping,price,cost,extra_charge,base_price,base_currency_code,price_currency_id,points,tax_class_id,date_available,weight,weight_class_id,length,width,height,length_class_id,subtract,minimum,sort_order,youtube_code,status,viewed,date_added,date_modified)
          $u1="UPDATE oc_product SET sku=".$sku.", quantity=".$quantity.", base_price=".$price.", price=".$price.", base_currency_code='".$arr3->product[$i]->data->currency."', weight=".$arr3->product[$i]->data->weight.", length=".$arr3->product[$i]->data->length.", width=".$arr3->product[$i]->data->width.", height=".$arr3->product[$i]->data->heigth.", minimum=1, sort_order=".$sort_order.", date_modified='".$arr3->product[$i]->data->date."', image='".$imgpath."'"." WHERE model = '".$model_u."'";
