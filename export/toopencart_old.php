...
require '/home/andriyyy/projects/Budsite/export/msql.inc.php';
...
$xmlnew = file_get_contents('/home/andriyyy/projects/Budsite/export/openc_imp.xml');
...
$quantity='\''.mysql_real_escape_string($arr3->product[$i]->data->quantity).'\'';
$price='\''.mysql_real_escape_string($arr3->product[$i]->data->price).'\'';
...
$pd="SELECT * FROM oc_product_description WHERE name=".$name;
 ...
 $prodesccnt=mysql_num_rows($prodesc);
    ...
    $imgname=str_replace('import_files/bf/','',$arr3->product[$i]->main->image);
...
$q="INSERT INTO oc_product (product_id,model,sku,upc,ean,jan,isbn,mpn,location,quantity,stock_status_id,image,manufacturer_id,shipping,price,cost,extra_charge,base_price,base_currency_code,price_currency_id,points,tax_class_id,date_available,weight,weight_class_id,length,width,height,length_class_id,subtract,minimum,sort_order,youtube_code,status,viewed,date_added,date_modified)
 VALUES ('', $model, $sku, '', '', '', '', '', '',$quantity,'','".$imgpath."',0,1,$price,$price,0,$price,$currency,1,0,0,$date,$weight,1,$length,$width,$heigth,1,0,$min_quantity,1,'',1,0,$date,$date  )";
...
if(!empty($pid)) {
    ...
    //subcategories
    ;
  if(!empty($arr3->product->main->subcategories)){
    for($d=0;$d<count($arr3->product->main->subcategories);$d++) {

        $subid=$subcategories[$d][id];
        echo $subid.'<br>';
        echo $pid.'<br>';
        if($subid!=0){
        $ptd="INSERT INTO oc_product_to_category (product_id,category_id) VALUES(".$pid.",".$subid.")";
...
*/
    //TODO category and subcategory INSERT OR UPDATE for products(oc_product_to_category,oc_product_to_store)
}
